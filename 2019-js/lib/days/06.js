...
const joinOrbitCounts = (orbitCounts1, orbitCounts2) => ({
  ...orbitCounts1,
  ...Object.fromEntries(
    Object.entries(orbitCounts2).map(([vertex, count]) => [
      vertex,
...
const calcOrbitCounts = (graph, start) =>
  [...(graph[start].keys() || [])]
    .map(dest =>
      Object.fromEntries([
        ...Object.entries(
          calcOrbitCounts(graph, dest)
        ).map(([vertex, orbitCount]) => [vertex, orbitCount + 1]),
        [dest, 1]
      ])
    )
...
exports.run = async input => {
  ...
  const part1 = sum(Object.values(calcOrbitCounts(graph, "COM")));
