const { readInput, range, compose, forEach } = require("./util");
...
const traceWire = (wirePath, fn) => {
  ...
  forEach(wirePath, path => {
    const direction = path[0];
    const distance = Number(path.slice(1));
    forEach(range(0, distance), moves[direction]);
  });
};
...
const main = async () => {
  const [input1, input2] = await readInput();
  const wire1Path = input1.split(",");
  const wire2Path = input2.split(",");
  ...
  console.log("Part 1:", manhattan(intersections[0].pos));

  // Part 2
  intersections.sort(({ steps: steps1 }, { steps: steps2 }) => steps1 - steps2);
  console.log("Part 2:", intersections[0].steps);
};
