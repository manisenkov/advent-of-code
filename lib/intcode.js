...
const commands = {
  [CMD_SUM]: async (mem, pointer, [modeArg1, modeArg2]) => {
  ...
  [CMD_MUL]: async (mem, pointer, [modeArg1, modeArg2]) => {
    const [arg1, arg2, pResult] = getArgs(mem, pointer, 3);
    const val1 = modeArg1 ? arg1 : mem[arg1];
    const val2 = modeArg2 ? arg2 : mem[arg2];
    return [update(mem, pResult, val1 * val2), pointer + 4];
  },
  [CMD_INPUT]: async (mem, pointer, _, input) => {
    const [pResult] = getArgs(mem, pointer, 1);
    const inputValue = await input();
    return [update(mem, pResult, inputValue), pointer + 2];
  },
  [CMD_OUTPUT]: async (mem, pointer, [modeArg], _, output) => {
    const [arg] = getArgs(mem, pointer, 1);
    await output(modeArg ? arg : mem[arg]);
    return [mem, pointer + 2];
  },
  [CMD_JUMP_IF_TRUE]: async (mem, pointer, [modeArg, modePointer]) => {
  ...
  [CMD_JUMP_IF_FALSE]: async (mem, pointer, [modeArg, modePointer]) => {
    const [arg, pNext] = getArgs(mem, pointer, 2);
    const checkVal = modeArg ? arg : mem[arg];
    const nextPointer = modePointer ? pNext : mem[pNext];
    return [mem, !checkVal ? nextPointer : pointer + 3];
  },
  [CMD_IF_LESS]: async (mem, pointer, [modeArg1, modeArg2]) => {
    const [arg1, arg2, pResult] = getArgs(mem, pointer, 3);
    const val1 = modeArg1 ? arg1 : mem[arg1];
    const val2 = modeArg2 ? arg2 : mem[arg2];
    return [update(mem, pResult, val1 < val2 ? 1 : 0), pointer + 4];
  },
  [CMD_IF_EQUAL]: async (mem, pointer, [modeArg1, modeArg2]) => {
    const [arg1, arg2, pResult] = getArgs(mem, pointer, 3);
    const val1 = modeArg1 ? arg1 : mem[arg1];
    const val2 = modeArg2 ? arg2 : mem[arg2];
    return [update(mem, pResult, val1 === val2 ? 1 : 0), pointer + 4];
...
module.exports.run = async (opcodes, input, output) => {
  let mem = [...opcodes];
  let pointer = 0;
  ...
  while (mem[pointer] !== CMD_TERMINATE) {
    ...
    [mem, pointer] = await cmdFn(mem, pointer, modes, input, output);
  }
  return mem[0];
};
