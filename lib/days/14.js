...
const { keys, entries, fromEntries } = require("../util");
...
const calcDistsFromFuel = recipes => {
  ...
  while (queue.length > 0) {
    ...
    for (const compChem of keys(recipes[chem].components)) {
...
const calcOreAmount = (recipes, dists, fuelAmount) => {
  ...
  while (state.size > 0) {
    ...
    const stateObj = fromEntries(stateRest.map(s => [s.chem, s.amount]));
    const toAdd = fromEntries(
      entries(components).map(([compChem, compAmount]) => [
        compChem,
    ...
    state = List(
      entries({ ...stateObj, ...toAdd }).map(([compChem, compAmount]) => ({
        chem: compChem,
        amount: compAmount
      }))
...
exports.run = async input => {
  const recipes = fromEntries(input.map(parseRecipe).map(r => [r.chem, r]));
