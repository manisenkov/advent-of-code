...
impl Solution<u64> for Day2022_16 {
    ...
    fn init(&mut self, input: &str) {
        ...
        for line in input.lines() {
            ...
            let destinations: Vec<String> = tunnel_input[22..]
                .split(", ")
            ...
            if flow_rate > 0 {
                self.valves.insert(valve_name.clone(), flow_rate);
            }
        }
        for valve in all_valves.keys() {
            for (k, dist) in calc_dists(&valve, &tunnels).iter() {
                self.dists.insert((valve.clone(), k.clone()), *dist);
            }
        }
    }
